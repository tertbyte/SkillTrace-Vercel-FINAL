...
export default function SecurityGate({children}){
 const [state,setState]=useState("loading"),[mode,setMode]=useState("government"),[email,setEmail]=useState(""),[password,setPassword]=useState(""),[error,setError]=useState(""),[busy,setBusy]=useState(false);
 ...
 if(mode==="user")return <UserPortal onGovernment={()=>setMode("government")}/>;
 return <div className="gov-auth-screen" style={screen}><div className="gov-auth-card" style={card}><div className="gov-auth-logo" style={logo}>S</div><div style={tabs}><button onClick={()=>setMode("government")}>Government</button><button onClick={()=>setMode("user")}>Candidate</button></div><div style={{fontSize:11,letterSpacing:2,color:"#6f849a",fontWeight:700}}>SECURE WORKSPACE</div><h1 style={{fontFamily:"Space Grotesk",margin:"8px 0",fontSize:28}}>SkillTrace</h1><p style={{color:"#8ca0b5",fontSize:12,lineHeight:1.6}}>Sign in to access Maharashtra workforce outcome intelligence.</p><form className="gov-auth-form" onSubmit={login} style={{display:"grid",gap:10,marginTop:20}}><input aria-label="Email" autoComplete="username" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Admin email" style={input}/><input aria-label="Password" autoComplete="current-password" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password" style={input}/>{error&&<div style={{color:"#ff9aa2",fontSize:10}}>{error}</div>}<button disabled={busy} style={button}>{busy?"Signing in…":"Sign in"}</button></form><small style={{color:"#5f7387",fontSize:9,marginTop:16,display:"block"}}>Protected by server-side authentication, secure cookies and rate limiting.</small></div></div>
}
