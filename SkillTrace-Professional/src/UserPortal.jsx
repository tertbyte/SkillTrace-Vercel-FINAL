...
function AnalysisPanel({result,notice,onEdit}){
 ...
 return <section className="candidate-panel analysis-panel"><div className="panel-title"><div><span>SKILLTRACE ANALYSIS</span><h3>Your next steps</h3></div><Sparkles/></div><p className="analysis-simple">{notice||"Simple guidance based on your profile."}</p><div className="analysis-levels">{blocks.map((block,i)=>{const lines=block.trim().split("\n").map(x=>x.trim()).filter(Boolean);const title=(lines[0]||"Analysis").replace(/^LEVEL \d+\s*[—-]?\s*/,"");const body=lines.slice(1).join(" ").replace(/[•\\n]/g," ").replace(/\s+/g," ").trim();return <div className="analysis-level" key={i}><b>{title}</b><div>{body}</div></div>})}</div><button className="user-primary" onClick={onEdit}>Edit my skills</button></section>
}